...
for endpoint_name in "${!endpoint_prefix[@]}"; do
  ...
  instances_as_base64=( $(curl -sL ${endpoint_url} | jq -r '.instances[] | select(.domain != "workstation.manta.systems" and .domain != "telemetry.manta.systems" and .domain != "telemetry.pelagos.systems") | @base64') )
  for x in ${instances_as_base64[@]}; do
    ...
    if [ "${cert_domains[0]}" = "${fqdn}" ]; then
      echo -e "[${endpoint_name}/${region}/${fqdn}] \e[32m${cert_domains[*]}\e[0m"
      ...
      echo -e "[${endpoint_name}/${region}/${fqdn}] \e[91m${cert_domains[*]}\e[0m"
    fi
  done
done
